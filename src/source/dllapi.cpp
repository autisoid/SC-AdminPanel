...
void ClientCommand(edict_t* pEntity) {
	...
	if (!strcmp(pcmd, "medic")) {
		...
		int uiFlag = 0;
		CALL_ANGELSCRIPT(pClientExecutedCommand, pEntity, &uiFlag);
		if (uiFlag != 0) {
...
void PM_Move(struct playermove_s* _PMove, int _Server) {
	if (_PMove == nullptr) {
		SET_META_RESULT(MRES_IGNORED);
		return;
	}
	int uiFlag = 0;
	CALL_ANGELSCRIPT(pPM_Move, _PMove, _Server, &uiFlag);
	if (uiFlag != 0)
		SET_META_RESULT(MRES_SUPERCEDE);
	else
		SET_META_RESULT(MRES_HANDLED);
...
void PM_Move_Post(struct playermove_s* _PMove, int _Server) {
	if (_PMove == nullptr) {
		SET_META_RESULT(MRES_IGNORED);
		return;
	}
	CALL_ANGELSCRIPT(pPost_PM_Move, _PMove, _Server);
	SET_META_RESULT(MRES_HANDLED);
}
