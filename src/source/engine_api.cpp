...
void HOOKED_MessageBegin(int msg_dest, int msg_type, const float* origin, edict_t* ed) {
	int uiFlag = 0;
	if (ASEXT_CallHook)
		(*ASEXT_CallHook)(g_AngelHook.pMessageBegin, 0, msg_dest, msg_type, &origin, ed, &uiFlag);
	if (uiFlag != 0) {
		SET_META_RESULT(MRES_SUPERCEDE);
		return;
	}
...
enginefuncs_t meta_engfuncs = 
{
	...
	HOOKED_MessageBegin,						// pfnMessageBegin()
	NULL,						// pfnMessageEnd()
	...
	int* interfaceVersion)
{
	...
	pEngfuncs = (enginefuncs_t*)malloc(sizeof(enginefuncs_t));
	memcpy(pEngfuncs, pengfuncsFromEngine, sizeof(enginefuncs_t));
